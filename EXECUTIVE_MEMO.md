...
|----------|------------|:------------:|
| Vague / unclassified codes | ~$417K | Yes |
| Spoilage claims | ~$153K | Yes |
| Label fines | ~$106K | Yes |
| Short-ship charges | ~$96K | Yes |
| Damaged goods | ~$96K | Yes |
| Pallet fines | ~$45K | Yes |
...
($361K) reference promotions not in Cinderhaven's calendar.
...
level. The gap between 20.9% and 30% is not about doing more
work; it is about classifying deductions well enough to know which
ones to contest.
...
2. **Audit spoilage and short-ship claims against shipping
   records.** $249K combined. Cross-reference against BOLs,
   carrier SLAs, and receiving logs. Where the carrier missed
   the window, file freight claims. Where the warehouse shorted
   the order, fix the process.

3. **Fix the label compliance gap.** $106K in label fines is a
   process failure, not a retailer relationship issue. Published
   specs, pre-ship QC, updated templates.

These three actions address the majority of the addressable total.
